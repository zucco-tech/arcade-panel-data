...
R = tempfile.mkdtemp(prefix="collision-")            # un dossier neuf, nettoye par le systeme
PORT_RA, ORIG = 46100, "170 170 170"
ADR_FBNEO, ADR_NEOGEO = 0x1234, 0x5678
def lampe(n):
    c = []
    for i in (1, 2):
...
def apprendre(systeme, adresse, core):
    ...
    for _ in range(5):
        ...
        if os.path.exists(APPRIS) and ("%s/memerom" % systeme) in json.load(open(APPRIS))["jeux"]: return
...
apprendre("neogeo", ADR_NEOGEO, "Geolith")
etat("endgame", "mame"); time.sleep(1.5)

b = json.load(open(APPRIS))["jeux"]
...
verifier("neogeo a la sienne",
         (b.get("neogeo/memerom") or {}).get("credits", {}).get("adresse") == ADR_NEOGEO)
verifier("chaque fiche note son coeur",
         b["fbneo/memerom"]["core"] == "FinalBurn Neo"
         and b["neogeo/memerom"]["core"] == "Geolith")
...
autre = cp.fiche_de(base, "fbneo", "memerom", "Geolith")
