...
# Les modules python : la ou credits(permanent).py et panneau(permanent).py les
# importent (sys.path <- userscripts/panneau-allinone), pas dans $N.
mkdir -p /recalbox/share/userscripts/panneau-allinone
for f in cablage.py couleurs.py reglages.py rzip.py; do [ -f \$f ] && mv -f \$f /recalbox/share/userscripts/panneau-allinone/\$f; done
